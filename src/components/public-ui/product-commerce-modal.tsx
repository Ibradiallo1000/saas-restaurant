...
export interface ProductCommerceModalProps
  extends Omit<PublicModalProps, "title" | "description" | "children" | "footer" | "maxWidth" | "headerContent"> {
...
export function ProductCommerceModal({
  ...
  return (
    <PublicModal
      ...
      maxWidth="lg"
      className={cn("max-h-[min(94dvh,840px)]", className)}
      headerClassName="p-0 pr-0"
      contentClassName={cn("px-[var(--space-4)] py-[var(--space-4)] sm:px-[var(--space-5)]", contentClassName)}
      footerClassName={cn("sticky bottom-0", footerClassName)}
      headerContent={
          ...
          <div className="flex h-[180px] w-full items-center justify-center overflow-hidden bg-[var(--surface-public-muted)] text-[var(--text-muted)] sm:h-[200px]">
            {showImage ? (
              ...
              <span aria-hidden="true" className="flex size-full items-center justify-center">
                {imageFallback ?? <ImageOff className="size-10 opacity-60" />}
              </span>
            )}
          </div>
      ...
      {...props}
    >
      <div aria-busy={loading || undefined} aria-disabled={disabled || undefined}>
        {children}
      </div>
    </PublicModal>
  )
