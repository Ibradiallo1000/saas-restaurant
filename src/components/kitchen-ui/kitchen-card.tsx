...
export const KitchenOrderCard = React.forwardRef<HTMLElement, KitchenOrderCardProps>(({ actions, className, context, density = "comfortable", destination, disabled = false, footer, headerExtra, items, loading = false, notes, onOpen, priority = "normal", reference, selected = false, status, timer, ...props }, ref) => (
  <article ref={ref} aria-busy={loading || undefined} aria-disabled={disabled || undefined} className={cn("min-w-0 rounded-[var(--radius-dashboard-card)] border border-[var(--order-border)] bg-[var(--order-surface)] p-[var(--kitchen-card-padding)] text-[var(--dashboard-title)] shadow-[var(--shadow-dashboard-surface)] transition-[border-color,background-color,box-shadow] [transition-duration:var(--motion-dashboard-hover)] motion-reduce:transition-none", priorityClasses[priority], selected && "ring-2 ring-[var(--order-border-active)] ring-offset-2", disabled && "opacity-60", density === "wallDisplay" && "[--kitchen-card-padding:var(--kitchen-card-padding-wall)]", className)} {...props}>
