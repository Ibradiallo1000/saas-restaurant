...
export interface DashboardChartCardProps extends Omit<React.HTMLAttributes<HTMLElement>, "title"> { title: React.ReactNode; description?: React.ReactNode; legend?: React.ReactNode; action?: React.ReactNode }
export const DashboardChartCard = React.forwardRef<HTMLElement, DashboardChartCardProps>(({ action, children, className, description, legend, title, ...props }, ref) => <article ref={ref} className={cn("rounded-[var(--radius-dashboard-widget)] border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] shadow-[var(--shadow-dashboard-surface)]", className)} {...props}><header className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--dashboard-divider)] px-4 py-3"><div><h2 className="text-sm font-semibold text-[var(--dashboard-title)]">{title}</h2>{description ? <p className="mt-0.5 text-xs text-[var(--dashboard-muted)]">{description}</p> : null}</div>{action}</header>{legend ? <div className="px-4 pt-3">{legend}</div> : null}<div className="p-4">{children}</div></article>)
