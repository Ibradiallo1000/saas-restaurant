  ...
  const loadHistory = React.useCallback(
    async ({ reset = false }: { reset?: boolean } = {}) => {
      ...
      try {
        ...
        setHistoryCursor(snapshot.docs[snapshot.docs.length - 1] ?? null);
        setHasMoreHistory(snapshot.docs.length === HISTORY_PAGE_SIZE);
      } finally {
        setHistoryLoading(false);
      }
    },
    [db, historyCursor, historyLoading, restaurantId]
  );
