...
import { TrendingUp, Clock, AlertTriangle, Wallet, LayoutDashboard, Calendar } from "lucide-react"
import { cn } from "@/lib/utils"
import { AnalyticsService, DashboardStats } from "@/services/analytics.service"
import { useRestaurant } from "@/design-system/context/RestaurantContext"
import { ChartSkeleton, DashboardRouteSkeleton } from "@/components/performance/route-skeletons"

const loadDashboardSalesChart = () => import("./DashboardSalesChart")

const DashboardSalesChart = dynamic(loadDashboardSalesChart, {
  ssr: false,
...
const EMPTY_DASHBOARD_STATS: DashboardStats = {
  sales: {
    today: { current: 0, previous: 0, percentageChange: 0 },
    week: { current: 0, previous: 0, percentageChange: 0 },
    month: { current: 0, previous: 0, percentageChange: 0 },
    breakdown: {
      cash: 0,
      mobileMoney: 0,
    },
...
export default function DashboardPage() {
  ...
  const [stats, setStats] = React.useState<DashboardStats>(EMPTY_DASHBOARD_STATS)
...
// 🔹 ANALYTICS
  ...
  React.useEffect(() => {
    if (!mounted || !analytics || !restaurantId) return

    let cancelled = false

    const cached = dashboardCache.get(restaurantId)
    ...
    const loadAnalytics = async () => {
      ...
      try {
        ...
        dashboardCache.set(restaurantId, {
          stats: newStats,
          chartData: trend,
  ...
  }, [analytics, mounted, restaurantId])
