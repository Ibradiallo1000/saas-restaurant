...
import { collection, query, doc, updateDoc, serverTimestamp, increment } from "firebase/firestore"
...
export default function InventoryPage() {
  ...
  const inventoryQuery = useMemoFirebase(() => {
    ...
    return query(collection(db, COLLECTION_NAMES.RESTAURANTS, restaurantId, COLLECTION_NAMES.INVENTORY))
  }, [db, restaurantId])
  ...
  const handleRestock = async (itemId: string) => {
    if (!db || !restaurantId || restockAmount <= 0) return
    setLoading(true)
    try {
      const itemRef = doc(db, COLLECTION_NAMES.RESTAURANTS, restaurantId, COLLECTION_NAMES.INVENTORY, itemId)
    ...
    <div className="space-y-6 pb-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="h-11 rounded-xl font-black border-primary/20 text-primary hover:bg-primary/5">
            <Filter className="mr-2 h-4 w-4" /> Filtres
          </Button>
          <Button className="h-11 rounded-xl bg-primary hover:bg-primary/90 font-black uppercase italic shadow-lg">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className={cn("border-none rounded-2xl shadow-lg", alertsCount > 0 ? "bg-destructive/5 ring-1 ring-destructive/20" : "bg-card/50")}>
        ...
        <Card className="border-none rounded-2xl shadow-lg bg-card/50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card className="border-none rounded-2xl shadow-xl bg-card/50 backdrop-blur-sm overflow-hidden">
                    ...
                    <TableCell className="text-right">
                      <Dialog>
                        <DialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-9 rounded-xl font-black text-primary hover:bg-primary/10">
                            Réapprovisionner
                          </Button>
                        </DialogTrigger>
                        <DialogContent className="rounded-2xl">
                          ...
                          <DialogFooter>
                            <Button className="w-full h-12 rounded-xl font-black uppercase italic shadow-lg" onClick={() => handleRestock(item.id)} disabled={loading}>
