...
export default function PlatformDashboard() {
  ...
  const requestsQuery = useMemoFirebase(() => {
    if (!db) return null;
    return query(collection(db, COLLECTION_NAMES.CONTACT_REQUESTS), orderBy('createdAt', 'desc'));
  }, [db]);

  const { data: restaurants, isLoading: isRestaurantsLoading } = useCollection(restaurantsQuery);
